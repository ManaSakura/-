...
class crawmodel:
    ...
    def __init__(self,user_agent=user_agent,ip='112.14.40.137:9091',url='https://www.baidu.com',isproxy=False):
        self.user_agent=user_agent
        self.ip=ip
        self.url=url
        self.isproxy=isproxy#是否设置代理
    ...
    def set_headers(self):
        self.headers = {'User-Agent': self.user_agent, 'Collection': 'Close'}
    def get_html_element(self,encoding_type='utf-8'):
        '''
        返回url的lxml.etree._Element对象及其转为bytes形式
        :param encoding_type: tostring的编码（encoding）方式
        :return:None
        '''
        self.set_headers()
        self.set_proxies()
        response=requests.get(self.url,headers=self.headers)
        self.html_element=etree.HTML(response.text)#只能带入text（str）,html_element为lxml.etree._Element类型
        self.html_bytes=etree.tostring(self.html_element,encoding=encoding_type)
    def get_html_node_byxpath(self, str_xpath_attr):
        # self.get_html_element()
        node = self.html_element.xpath(str_xpath_attr)
        return node
    def write_html(self,savepath):
        self.url_html_savepath=savepath
        with open(self.url_html_savepath, 'wb') as f:
            f.write(self.html_bytes)
        # print('htmlbytes.html', '已写入完成')
